...
<div class="sl-mainpanel">
    ...
    <div class="row row-sm">
      <div class="col-sm-6 col-xl-4">
        <div class="card pd-20 bg-primary">
          ...
          <?php
            $decedes = DB::table('participants')->where('decede',1)->get();
            $cotisation = DB::table('reunions')->sum('montant_obtenu');
            $first = DB::table('reunions')->orderby('montant_obtenu','DESC')->first();
            $montants = DB::table('reunions')->select('montant_obtenu','date')->get();
            $sum_prevu = count(DB::table('reunions')->get()) * count(DB::table('participants')->get())* 1000;
            $mt = array();
          ...
          <div class="d-flex align-items-center justify-content-between mg-t-15 bd-t bd-white-2 pd-t-10">
            ...
            <div>
              <span class="tx-11 tx-white-6">Reste à payer</span>
              <h6 class="tx-white mg-b-0">{{number_format(($sum_prevu - $cotisation + $maj),0,',','.') }}</h6>
            </div>
            @endif
          </div><!-- -->
        </div><!-- card -->
  ...
  <div class="carousel-inner">
    ...
    @endforeach
      ...
      <?php
         $reunionBilan = DB::table('reunions')
                              ->select(DB::raw('SUM(montant_obtenu) as total'),DB::raw('YEAR(date) AS year'))
                              ->groupBy('year')
                              ->get();
        $date = array();
        $montant = array();
        foreach ($reunionBilan as $rb){
          array_push($montant,$rb->total);
          array_push($date,$rb->year);
        }
        $montant = implode(',',$montant);
        $date = implode(',',$date);
       ?>
       <div class="row row-sm" style="margin-top:40px;margin-left:auto;margin-right:auto;">
           <div class="col-xl-12">
             <div class="card pd-20 pd-sm-40">
               <h6 class="card-body-title">BILAN IMAGE</h6>
               <input type="hidden" value="{{$date}}" class="date">
               <input type="hidden" value="{{$montant}}" class="montant">
               <p class="mg-b-20 mg-sm-b-30 text-danger">TOTAL DES MONTANTS COLLECTES PAR ANNEE AU COURS DES REUNIONS MENSUELLES.</p>
...
<script src="{{asset('public/backend/lib/chart.js/Chart.js')}}"></script>
  ...
  var montant = document.getElementsByClassName('montant')[0].value;

  date = date.split(',');
  montant = montant.split(',');
   var max = montant.reduce(function(a,b){
     return Math.max(a,b);
   });
